...
pub fn sighash(namespace: &str, name: &str) -> [u8; 8] {
    // Anchor derives instruction discriminators using a single-colon
    // namespace separator (e.g. "global:my_instruction"). Explicitly build the
    // preimage this way so that downstream consumers, like the Unity and CLI
    // helpers, hash the exact same string.
    let mut preimage = String::with_capacity(namespace.len() + name.len() + 1);
    preimage.push_str(namespace);
    preimage.push(':');
